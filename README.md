...
**Accuracy and metacognitive monitoring are negatively correlated across frontier LLMs — but the dissociation is trainable.** Across 7 frontier models and 50 hand-crafted tasks, the global TDR–accuracy correlation is **r = −0.84** (95% CI [−0.98, −0.24], p = 0.018, LOO-stable). Three of five families independently reproduce the sign-flip at p < 0.04.

The top-accuracy models (claude-opus-4-5 at 100%, gpt-4o at 98.2%) rank **last** on metacognitive monitoring. But `claude-haiku-4-5-20251001` — Anthropic's smallest, most recent model — tops the Metacognitive Index at MI = 0.615 despite 96.3% accuracy, breaking the pattern. **Haiku proves the capability/monitoring trade-off is not inherent; it is a training artifact** — which makes it tractable for the AI-alignment research agenda.
...
python v3_statistical_analysis.py
python v3_robustness.py
```

For local evaluation demo (no API key required):
...
├── v3_robustness.py             # Bootstrap CI + permutation p-values (10 000 resamples each)
├── v3_generational_analysis.py  # Within-Anthropic lineage comparison
...
│   ├── v3_tasks_50.json         # Task mirror for the Space
│   └── requirements.txt
├── data/                        # Raw evaluation records & reports
├── tasks.json                   # Legacy v1 task set (superseded by v3_tasks_50.json)
├── kaggle_tasks.json            # Legacy v2 task set (superseded)
...
- LOO (leave-one-out) stability verified: removing any single model from the n=7 pool does not flip the sign of the key correlations
...
|---|---|---|---|
| Global TDR vs. accuracy | [−0.94, −0.81] | 0.81 | ✓ |
| confidence_inversion | [+0.86, +0.93] | 0.86 | ✓ |
| expertise_trap | [−0.86, −0.74] | 0.74 | ✓ |
| forced_abstention | [−0.89, −0.76] | 0.76 | ✓ |
...
**Confidence intervals** were computed via Fisher z-transform (standard for bounded correlations at small n). With n = 7 models, the SE of z is 1/√(n−3) = 0.500; the 95% Wald interval is back-transformed via tanh. The width of the global CI (−0.98 to −0.24) reflects the small sample — but both endpoints are negative, so the direction is unambiguous.
...
**Insight 3 — MI spread is 0.208** (0.407 to 0.615), providing meaningful discriminatory power. No model saturates the benchmark.
