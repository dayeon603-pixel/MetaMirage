...
**Robustness checks.** Cross-judge: Claude responses (n = 150) re-judged by `claude-opus-4-5`; κ 0.65–0.97 across 6 dimensions, Pearson(judges) = 0.88, **haiku #1 holds under both**. Rubric-weight sensitivity: across all 36 (w_rubric, w_abstain, w_expert) triplets on a 0.1-grid, **94% negative, 81% p < 0.05**. n_effective: of 50 tasks, 12 are control-only; sign-flip driven by 38 mirage-bearing tasks. **Novelty audit** (char-5-gram Jaccard vs MMLU proxy): max 0.28, mean 0.05 — well below paraphrase threshold. **Item discrimination** (3-Anthropic-model subset): 10 tasks score-spread ≥ 0.3, 16 ≥ 0.1; control items appropriately uniform (spread = 0).
